...
void MainWindow::assign_blk_18()
{
    ...
    QString labels[12] = {"28", "4", "64", "17", "5", "97", "98", "50", "10", "18", "35", "99"};
    xPositions[9] = leftX + 900; // Shift right from blkno 2 mod 4
    yPositions[9] = leftYPositions[2]; // Align vertically
    ...
    QGraphicsItem *lines[] = {
        new BezierCurvyLine(
            QPointF(freeleftX + 95, freeleftYPositions + 25),  // Start at Freelist box
            QPointF(freeleftX + 200, freeleftYPositions - 50), // Control point (curved path)
            QPointF(300 - 150, 225 + 200),                     // Control point (curved path)
            QPointF(300, 225)                                  // End at Box 5
        ),
    ...
    // QGraphicsTextItem *delayLabel = new QGraphicsTextItem("writing");
    // delayLabel->setDefaultTextColor(Qt::white); // Set text color
    // delayLabel->setFont(QFont("Arial", 12, QFont::Bold)); // Set font style
    // delayLabel->setPos(xPositions[9] + 30, yPositions[9] + 60); // Position slightly below box "3"
    // scene->addItem(delayLabel);

    // QGraphicsTextItem *delayLabel2 = new QGraphicsTextItem("writing");
